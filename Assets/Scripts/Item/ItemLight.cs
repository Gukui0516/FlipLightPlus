...
public class ItemLight : MonoBehaviour, IInvertibleColor
{
    // 아이템에서 빛 뿜어져 나오는 거
    ...
    private void Awake()
    {
        ...
        originSpriteRenderer = GetComponent<SpriteRenderer>();
    ...
    /// <summary>아이템이 플레이어에게 먹힘 순간 호출해주세요.</summary>
    ...
    public void SetInvertedColor()
    {
        ...
        if (originSpriteRenderer != null)
            originSpriteRenderer.color = invertedColor;
    }
    ...
    public void SetOriginalColor()
    {
        currentColor = color;
        if (originSpriteRenderer != null)
            originSpriteRenderer.color = color;
    }
    ...
    private void SpawnFlyingStreak()
    {
        ...
        sr.color = currentColor; // 현재 색상 사용
